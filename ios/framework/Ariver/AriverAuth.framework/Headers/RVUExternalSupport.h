...
EX_MAIN_CALL_DECLARE(ArviverAuth)
#ifdef __cplusplus
}
#endif
...
#define MAIN_CALL ExMainCall_ArviverAuth // MAIN_CALL(tag, delayTime, priority, block)
//#error ThisModuleName需要替换为自己模块名（区分大小写），替换后注释该错误提示。------ （第3个替换）------
#define MAINA_CALL ExMainACall_ArviverAuth  // MAINA_CALL(tag, delayTime, priority, block)
