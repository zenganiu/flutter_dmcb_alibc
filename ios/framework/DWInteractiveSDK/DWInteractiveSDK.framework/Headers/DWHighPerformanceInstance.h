...
#import <DWInteractiveSDK/DWHighPerformanceVideoController.h>
...
@interface DWHighPerformanceInstance : NSObject
 ...
 * playerConfig字段可以为nil，目前支持以下参数：
 1. disableAudio: 选择是否禁掉音频（禁掉音频，即播放器内核会禁止音频的解码和播放），默认为YES，即禁掉音频；
 2. disableTimeUpdate: 选择是否实时上报视频播放时间进度，默认为YES，即不实时上报;
 3. usePlayerManager: 选择是否使用播放器内核多实例管理，默认YES，即使用；
 4. useCustomVideoURL: 选择是否使用特殊转码的视频地址，默认YES，即使用；
 5. useEGLRenderer: 选择是否在TaobaoPlayer下使用EGL渲染，默认NO，即不用EGL；
 6. mute:默认NO，即不静音
 *
 *
 */
 ...
 * 播放模式，图片或者视频，默认图片；
 */
@property (nonatomic, assign) DWHPContentMode contentMode;
